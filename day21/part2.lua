...
-- Warning: this edits both the player and the boss hit point.
...
function will_player_win2(player)
  local damage_to_boss   = math.max(player.damage -   boss.armor, 1)
  local damage_to_player =   math.max(boss.damage - player.armor, 1)
  ...
  for a = 0, #armor do
    for r1 = 0, #rings do
      for r2 = -1, r1 - 1 do
        ...
        if not will_player_win2(p) and c > best_cost then
          best_cost = c
          best_case = {w, a, r1, r2}
        end
      end
...
-- I used this to help find a bug:
print('best case:')
print_table(best_case)
io.write('w : ')
print_table(weapons[best_case[1]])
io.write('a : ')
print_table(armor[best_case[2]])
io.write('r1: ')
print_table(rings[best_case[3]])
io.write('r2: ')
print_table(rings[best_case[4]])
